...
and let $\calG = \{\, A \subseteq S : \mbox{$A$ is infinite} \,\}$ be
the set of all infinite subsets of $S$, then the
...
Moreover, if we giving $S$ the discrete topology we can give $\beta S$,
...
somewhat difficult to understand, we choose to give a some 
motivation on how this concept occurs before explicitly defining it. 
...
\begin{proof}
  ...
  \ra_{n=1}^\infty$. 
  Pick $c \in \bbN$ such that $G \subseteq \{1, 2, \ldots, c\}$.
  ...
  Therefore it follows that $x_{n+1} - x_n \le x_m - x_n \le c$, and 
...
We define the \textsl{gap size of $F$} as $\max\bigr\{\, x_{j+1} - x_j : j
\in \{1, 2, \ldots, n-1\} \,\bigl\}$.
We will temporarily denote the gap size of $F$ by $g(F)$. 
We call a set $A \subseteq \bbN$ \textsl{piecewise syndetic} if and
only if there exists $b \in \bbN$ such that for all $N \in \bbN$,
there exists $F \in \Pf(A)$ with $|F| \ge N$ and $g(F) \le b$.
...
\begin{proof}
  ...
  We may assume that $|B| = N$. 
  Enumerate $B$ as a strictly increasing sequence $x_1 < x_2 < \cdots
  < x_N$
  ...
  \ldots, x_N\}$.
  (We can do this since $|B| = 3 |F| \max F$.)
  Let $f \in F$, then $f + x \in \{x_1, x_1 + 1, \ldots, x_N
  \}$ and we can pick $n \in \bbN$ with $x_n \le f+x < x_{n+1}$. 
  Since $x_{n+1} - x_n \le b$, there exists $t \in G$ such that $t +
  f + x = x_{n+1}$. 
...
\begin{prop}
  Let $A \subseteq \bbN$.
  If $A$ is syndetic, then $A$ is piecewise syndetic.
\end{prop}
\begin{proof}
  This follows directly from Proposition \ref{prop:syn}
\end{proof}
