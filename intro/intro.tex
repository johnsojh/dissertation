...
With such a ``definition'', it's a fact that studying partition
regular $(S, \calG)$ is equivalent to studying certain ultrafilters on
$S$. 
...
still forms the bases for much of the contemporary research in the
field.
...
\begin{thm}
  Let $S$ be a semigroup and let $r$, $\ell \in \bbN$.
  If $S = \bigcup_{i=1}^r C_i$, then there exist $i \in \{1, 2,
  \ldots, r\}$ and $a$, $d \in S$ such that $\{\, a , ad, \ldots, ad^\ell
  \,\} \subseteq C_i$. 
\end{thm}
\begin{proof}
  Fix $x \in S$.
  ...
  \in \bbN$ such that $\{\, b, b+c, \ldots, b+\ell c \,\} \subseteq B_i$. 
