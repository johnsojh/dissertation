...
A \textsl{semigroup} is a pair $(S, \cdot)$ such that $S$ is nonempty
and $\cdot$ is an associate binary operation on $S$, that is, $x \cdot
(y \cdot z) = (x \cdot y) \cdot z$ for all $x$, $y$, and $z \in S$.
As usual in mathematics, we will often suppress the explicit mention
of our associate binary operation $\cdot$ in algebraic formulas and
simply write that $S$ is a semigroup. 
...
\begin{fact}
  ...
  \bigtimes_{i \in I} S_i$.
  Define a binary operation on $S$ coordinatewise.
...
We this assumption we automatically gain the following facts.

\begin{fact}
  Let $S$ be a compact right-topological semigroup.
  \begin{itemize}
    \item[(a)] $S$ contains an idempotent.

    \item[(b)] The smallest ideal of $S$ exists.
  \end{itemize}
\end{fact}
\begin{proof}
  Proofs for (a) and (b) can be found in \cite[Theorem
  2.5]{Hindman:1998fk} \cite[Theorem 2.8]{Hindman:1998fk},
  respectively.
\end{proof}
...
\begin{fact}
  ...
  Given $S$ the product topology and define a binary operation on $S$
  coordinatewise. 
  \begin{itemize}
...
weak form of a ``nonconstructive'' axiom of choice.
See \cite{Rav:1977ys} for interesting discussion about ``combinatorial
principles'' equivalent to the existence of nonprincipal
ultrafilters.)
...
topology on $\beta S$.
In fact, this topology is the Stone-\v{C}ech compactification of $S$
\cite[Theorem 3.27]{Hindman:1998fk}. 
...
We shall constantly use the following facts.
\begin{fact}
