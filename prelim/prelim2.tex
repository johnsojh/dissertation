...
\end{prop}
\begin{proof}
  Let $\calC$ be the cofinite filter on $\bbN$, let $E$ represent the
  even numbers and $O$ the odd numbers.
  Then $E$ and $O$ are members of $G(\calC)$.
  Let $\calF_1$ represent the filter generated by $\calC \cup \{O\}$
  and $\calF_2$ represent the filter generated by $\calC \cup \{E\}$.
  A straightforward check shows that $G(\calF_1) \cap G(\calF_2)$ is a
  grill base. 
  However $E \cup O = \bbN \in G(\calF_1) \cap G(\calF_2)$ but $E$ and
  $O$ are not members of $G(\calF_1) \cap G(\calF_2)$.
\end{proof}
