...
\begin{defn}
  Let $X$ be a nonempty set and $\calS$ a grill subbase on $X$.
  We say that the $\calS$ is \textsl{partition regular} if and only if whenever $\cal$ is a finite collection of sets with $\bigcap\calA \in \calS$, then there exist $A \in \calA$ and $B \in \calS$ such that $B \subseteq A$.
\end{defn}
