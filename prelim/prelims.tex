...
\begin{proof}
  ...
  The fact $\AP$ is easy to see using the following common technique.
...
We are now in a position to formulate our `hidden' problem mentioned earlier, and we start by simply modifying the definition of an $AP$-set, Theorem \ref{thm:ap-partition-reg}, and the definition of $\AP$ to be applicable to a semigroup.
...
\end{defn}
\begin{rmk}
  The reason we don't call an algebraic line by the perhaps more fitting name of `algebraic progression' is because we shall soon see that algebraic lines are not as nice as arithmetic progressions.
\end{rmk}
...
\begin{proof}
  ...
  Let $A \in pq$, then $\{\, x \in S : x^{-1}A \in q \,\} \in p$. 
  Pick $x \in S$ such that $x^{-1}A \in q$. 
...
\begin{proof}
  ...
  We show that $Ba$ is not an $AL$-set by showing that $Ba$ doesn't contain even a 2-term algebraic lines. 
  Suppose there exists $c$, $d \in S$ with $\{c, cd\} \subseteq Ba$. 
  Pick positive integers $i$ and $j$ such that $c = b^ia$ and $cd = b^ja$.
  Then $b^ja = cd = b^iad$.
  If $i < j$, then $b^{j-1}a = ad$, a contraction.
...
One of these modifications is the notion of a piecewise syndetic set.
However sine the definition of a piecewise syndetic set is, initially, somewhat difficult to understand, we choose to give some motivation on how this concept can be `naturally' derived before explicitly defining it.
...
\begin{proof}
  ...
  We can then apply (a) to pick $i \in \{1, 2, \ldots, b\}$ such that for all $\ell \in \bbN$, the set $-i + A$ contains a $k$-term arithmetic progression. 
...
\begin{rmk}
  ...
  \cite[Theorem 2.5]{Landman:2004qf} contains a more complete list of statements equivalent to van der Waerden's Theorem with proofs.
\end{rmk}
...
\subsection{Piecewise Syndetic Sets and Central Sets}
