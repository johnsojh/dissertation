...
(We can summarize this observation into an elliptic misleading proverb that qualitative is easier than quantitive.)
...
We want to produce a reasonable definition for a collection $\calF$ of subsets of $X$ such that members of $\calF$ are considered as ``large'' subsets of $X$.
...
\begin{rmk}
  ...
  (It's probably easier to prove $(B) \implies (A)$, $(A) \implies (C)$, $(C) \implies (D)$, and $(D) \implies (B)$.)
\end{rmk}
...
Without some reasonable strong version of the axiom of choice there may be no nontrivial maximal filters at all.
...
\endnote{
  We have been unable to verify (or falsify!) the assertions in the sixth and seventh paragraphs of section II and the assertion in section IV in \cite{Choquet:1947uq}.
  One of the assertions in section II is that $\calB = \bigcap\{\, \calG : \mbox{$\calG$ is a grill with $\calB \subseteq \calG$} \,\}$.
}
...
While grill bases are easier to describe then grills we shall see that in Proposition \ref{prop:GrillB}
...
\begin{proof}
  ...
  In order to show to it suffices to show that for every $B \in \calB$ and every finite nonempty set $\calA \subseteq F(\calB) \cup \{X \setminus A\}$ we have $B \cap \bigcap\calA \ne \emptyset$.
  ...
  However $G(\calF)$ is grill that contains $\calB$ with $A \in G(\calF)$ and $A \not\in G(\calF)$, a contradiction. 
\end{proof}
