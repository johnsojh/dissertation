...
We call $L \subseteq S$ a \textsl{left ideal of $S$} if and only if $L$ is nonempty and for all $x \in S$, $\lambda_x[L] \subseteq L$. 
We call $R \subseteq S$ a \textsl{right ideal of $S$} if and only if $R$ is nonempty and for all $x \in S$, $\rho_x[R] \subseteq R$. 
...
\begin{proof}
  ...
  Let $\bigcup_{i=1}^r C_i$ be an $AP$-set, in particular, $\bigcup_{i=1}^r C_i$ is infinite. 
  ...
  Hence suppose instead that $\max\{x_{n+1} - x_n : n \in \bbN\}$ is infinite, that is, for every $k \in \bbN$, there exists $n \in \bbN$ with $x_{n+1} - x_n \ge k+2$. 
  Therefore for every $k \in \bbN$, there exists $n \in \bbN$ with $\{x_n + 1, x_n + 2, \ldots, x_n +(k+1) \} \subseteq \bbN \setminus C_r$. 
