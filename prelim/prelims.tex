...
\endnote{
  Almost a decade after van der Waerden published his proof, Paul Erd\H{o}s and Paul Tur\'{a}n in \cite{Erdos:1936fk} started investigating when a finite set of natural numbers contain would contain a 3-term arithmetic progression.
  One of their conjectures contained in this paper is the following result: For every $\delta > 0$ there exists a natural number $N$ such that if $A \subseteq \{1, 2, \ldots, N\}$ with $|A| \ge \delta N$, then $A$ contains a 3-term arithmetic progression. 
