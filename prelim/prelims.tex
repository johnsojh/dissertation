...
Bartel Leendert van der Waerden in \cite{Van-der-Waerden:1927fk} proved (the remarkable assertion) that if the natural numbers are divided into finitely many classes, then at least one class contains a $k$-term arithmetic progression for every natural number $k$.
An equivalent way to state this theorem is as follows:  
Pick any finite number of colors and give each natural number a particular color.
Then for each natural number $k$ there is a monochromatic $k$-term arithmetic progression, that is, there is a $k$-term arithmetic progression such that each of the $k$ terms share the same color.%
\endnote{
  A more careful reading shows that our first formulation and second formulation, using the `color terminology', of van der Waerden's Theorem are slightly different.
  In the first formulation, we assert that one class in our division of $\bbN$ contains a $k$-term arithmetic progression for every natural number $k$.
  While in the second formulation, we assert that for every natural number $k$ there is a class which contains a $k$-term arithmetic progression.
  These two formulations, despite the interchange between the quantifiers `there exists' and `for all', are equivalent.
  One direction in this equivalence is easy, and the other direction follows from the pigeonhole principle. 
}
This theorem still forms the basis for much of the contemporary research in the field of Ramsey Theory%
\endnote{
...
We call a left ideal a \textsl{minimal left ideal} if and only if the only left ideal it contains is itself.
...
\endnote{
  ...
  Suschkewitsch, based off of the description given by \cite{Hollings:2009uq}, in \cite{Suschkewitsch:1928kx} proved that all finite semigroups have a smallest ideal and using this fact was able to produce a structure theorem for finite semigroups. 
}
...
We are now in a position to formulate our `hidden' problem mentioned earlier. 

\begin{defn}
  We call $A \subseteq \bbN$ an \textsl{$AP$-set} if and only if for every $k \in \bbN$, $A$ contains a $k$-term arithmetic progression. 
\end{defn}

With this definition we can formulate van der Waerden's theorem as the assertion that if $\bbN$ is divided into finitely many classes, then at least one class is an $AP$-set. 
We wish to show the superficially stronger result that if an $AP$-sets is divided into finitely many classes, then at least one class is an $AP$-set. 
The standard proof, and the one we give, follows from yet another equivalent form of van der Werden's Theorem. 

\begin{vdw}
  For every $r$, $k \in \bbN$, there exists a natural number $N$ such that if $\{1, 2, \ldots, N\} = \bigcup_{i=1}^r C_i$, then there exists $i \in \{1, 2, \ldots, r\}$ such that $C_i$ contains a $k$-term arithmetic progression.
\end{vdw}
...
\end{thm}
\begin{proof}
  By the pigeonhole principle it suffices to show that for each $k \in \bbN$, there exists $i \in \{1, 2, \ldots, r\}$ such that $C_i$ contains a $k$-term arithmetic progression. 

  By van der Waerden's Theorem, pick a natural number $N$ such that if $\{1, 2, \ldots, N\}$ is divided into $r$ pieces, then at least one piece contains a $k$-term arithmetic progression. 
  Pick $a$, $d \in \bbN$ such that $\{a, a+d, \ldots, a+Nd\} \subseteq A$. 
  For each $i \in \{1, 2, \ldots, r\}$ define $B_i = \bigl\{\, j \in \{1, 2, \ldots, N\} : a+jd \in C_i  \,\bigr\}$.
  Then $\{1, 2, \ldots, N\} = \bigcup_{i=1}^r B_i$. 
  Pick $i \in \{1, 2, \ldots, r\}$ and $c$, $b \in \bbN$ such that $\{c, c+b, \ldots, c+(k-1)b\} \subseteq B_i$. 

  We claim that $C_i$ has a $k$-term arithmetic progression. 
  By definition of $B_i$ we have that for all $j \in \{0, 1, \ldots, k-1\}$, $a + cd + jbd = a + (c + jb)d \in C_i$, that is, $C_i$ contains a $k$-term arithmetic progression.
\end{proof}
