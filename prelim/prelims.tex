...
This theorem can be rewritten to apply to any semigroup, where we replace the arithmetic progression $\{a, a+d, ..., a+(k-1)d\}$ by $\{a, ad, ..., ad^{k-1}\}$, and as observed in the Introduction of \cite{Bergelson:1992fk},this `generalization' follows directly from van der Waerden's Theorem itself. 
...
\end{thm}
\begin{proof}
  Pick a positive integer number $N$ as guaranteed by van der Waerden's Theorem for $r$ and $k$.
...
\endnote{
  Let $S$ be a nonempty set and let $\Phi$ be some `property' of $S$.
  We call $\Phi$ \textsl{partition regular} if and only if whenever $A \subseteq S$ with property $\Phi$, $r$ is a positive integer, and $A = \bigcup_{i=1}^r C_i$, then there exists $i \in \{1, 2, \ldots, r\}$ such that $C_i$ has property $\Phi$. 
  It's a fact that partition regular properties interacts nontrivially with $\beta S$, see \cite[Theorem 3.11]{Hindman:1998fk} and \cite[Theorem 5.7]{Hindman:1998fk}.
  In this dissertation we are typically only interested in the case when $S$ is a semigroup and $\Phi$ is defined in terms of the algebraic structure of $S$.
...
With Theorem \ref{thm:ap-partition-reg} and \cite[Theorem 3.11]{Hindman:1998fk} we have that the following subset of $\beta \bbN$ is nonempty.
