...
\begin{thm}
  ...
  \begin{itemize}
    ...
    \item[(c)]
      For every positive integers $b$, $c$, and $k$, there exists a positive integer $N$ such that if $\{b+c, b+2c, \ldots, b+Nc\} = C_1 \cup C_2$, then either $C_1$ or $C_2$ contains a $k$-term arithmetic progression.
...
\begin{proof}
  ...
  \textsl{(b) $\implies$ (c).}
  This proof is similar to Theorem \ref{thm:semigrp-vdw}. 

  \textsl{(c) $\implies$ (d).}
  Let $k$ and $r$ be a positive integers.
  If $r=1$, then \textsl{(d)} is trivially true.
  If $r=2$, then \textsl{(d)} is true by assumption.
  So we may assume that $r>2$. 

  By assumption for each $b$,$c \in \bbN$ pick $N(b,c) \in \bbN$ such that if $\{b, b+c, \ldots, b+(N(b,c) - 1)c\} = C_1 \cup C_2$, then either $C_1$ or $C_2$ contains a $k$-term arithmetic progression.

  By assumption pick $N$ such that if $\{1, 2, \ldots, N\} = \bigcup_{i=1}^r C_i$, then either $C_1$ or $\bigcup_{i=2}^r C_i$ contains a $k$-term arithmetic progression.
  If $C_1$ contains a $k$-term arithmetic progression, then we are done.
  If $C_1$ does not contain a $k$-term arithmetic progression, then pick $b$ and $c$ such that $\{b, b+c, \ldots, b+(k-1)c\} \subseteq \bigcup_{i=2}^r C_i$. 
...
We wish to translate statement Theorem \ref{thm:vdwEqSyn} (g) into the language of semigroups.
